 ...
 */
class MUHorizontalPager : ViewPager, MUPageControlListener {
    ...
    var muPageControl: MUPageControl? = null
        set(MUPageControl) {
            ...
            if (muPageControl != null) {
                ...
                muPageControl?.pageControlListener = this
    ...
    private inner class MyOnPageChangeListener(private val horizontalPager: MUHorizontalPager) : OnPageChangeListener {
        ...
        override fun onPageScrolled(position: Int, positionOffset: Float, positionOffsetPixels: Int) {
            val newPosition: Int = when(position == mCurrentIndex) {
                true -> if (positionOffset > UPDATE_THRESHOLD) mCurrentIndex + 1 else mCurrentIndex
                false -> if (positionOffset < 1 - UPDATE_THRESHOLD) mCurrentIndex - 1 else mCurrentIndex
            }

            if (newPosition > -1) {
                if (null != muPageControl) {
                    muPageControl?.currentPosition = newPosition
                }

                if (null != slideListener) {
                    slideListener?.invoke(horizontalPager, newPosition)
                }
            }
        }
    ...
    private fun init(context: Context) {
        ...
        if (isInEditMode) {
            ...
            muHeader.title = context.getString(R.string.title)
            muHeader.detail = context.getString(R.string.detail)
    ...
    override fun clickOnIndex(muPageControl: MUPageControl, index: Int) {
